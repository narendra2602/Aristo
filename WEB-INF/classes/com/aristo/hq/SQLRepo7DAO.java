...
//	    	   terrec = "Select distinct(gp_code),gp_name from "+tblnm1+" where gp_code=? and mkt_year=? group by gp_code order by gp_code";
    	   terrec = "Select distinct(gp_code),gp_name from "+tblnm1+" where gp_code=? order by gp_code";
...
//    	   ts1.setInt(2,eyear);
    	   trec = ts1.executeQuery();
            ...
            String query1 ="Select h.ter_code,h.ter_name,u.depo_code from "+tblnm2+" as h,user_ter as u " +
...
*/		      query3 = "Select sum("+unm+"),sum(r"+unm+"),sum(e"+unm+"),sum(b"+unm+"),sum(s"+unm+"),sum(p"+unm+"),sum("+vnm+"),sum(r"+vnm+"),sum(e"+vnm+"),sum(b"+vnm+"),sum(s"+vnm+"),sum(p"+vnm+") from "+tblnm+" where tr_cd=? "+
			...
			System.out.println("========Exception in HQ-SQLRepo7DAO.getAllHQrepo6 " + e); 
		}
		finally 
		{
		try {
	           ...
	           if(con != null){con.close();}
				...
				System.out.println("--Exception in HQ-SQLRepo7DAO.Connection.close "+e);
			  }
		}
