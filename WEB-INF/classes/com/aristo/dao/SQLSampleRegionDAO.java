...
public class SQLSampleRegionDAO implements RegionDAO {
	...
	public List getAllRegion(Connection con,int depo,String typ,int eyear) { 
		...
		try { 
            ...
            String tblnm=null;
            String tblnm1=null;
        	tblnm=(typ+"_sregmst").toLowerCase();
    	    tblnm1=(typ+"_sareamst").toLowerCase();

			String query = "select r.reg_code,r.name,a.area_name from " + tblnm +" as r, "+tblnm1 
			+" as a where r.area_code=a.area_cd and r.depo_code=a.depo_code and r.depo_code=? " 
			+" and r.mkt_year=? and a.mkt_year=? ";
			...
			System.out.println("========Exception in SQLSampleRegionDAO.getAllRegion " + e);
		}
			...
			try
			{
	           ...
	           if(con != null){con.close();}
				...
				System.out.println("-------------Exception in SQLSampleRegionDAO.Connection.close "+e);
			  }
		}		
		return data;
	}
 ...
 public int updateRegion(String typ,List reg,Connection con)
   {
	   ...
	   ResultSet rs=null;
    	...
    	String query ="update "+tblnm+" set NAME=?,AREA_CODE=?,TXT=? where DEPO_CODE=? and Reg_Code=? and mkt_year=?";
		ps = con.prepareStatement(query);
        ...
        Iterator it = reg.iterator();
       	while(it.hasNext()) 
			...
			if (rs.next())
			{
				...
				ps.setInt(4,r.getDepo_code());
				ps.setInt(5, r.getReg_code());
				ps.setInt(6, r.getMkt_year());
   		...
   		ps2.close();
			...
			System.out.println("-------------Exception in SQLSampleRegionDAO.update " + ex);
			i=-1;
		}
		finally {
			try {
