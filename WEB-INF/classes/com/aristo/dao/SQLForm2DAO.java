...
public class SQLForm2DAO {

	public List getForm2(Connection con, int code, int emon,int eyear,int depo_code,String tp,int uid,String utype) { 
...
/////////////////////////////Headquater Master Ki Query//////////////////////////////////
			...
			" from( "+
			" select ar_cd,area_name,rg_cd,reg_name,tr_cd,ter_name,"+grp+",sum(lysval) lysval, sum(tarval) tarval,sum(salval) salval from "+tblnm+"  "+ 
			" where mkt_year = ?   and  mnth_code = ?  and "+grp+" = ?  "+
			" and depo_CODE =? "+
			" group by ar_cd,area_name,rg_cd,reg_name,tr_cd,ter_name,"+grp+") as a "+
			" left join "+
			" (select ar_cd,area_name,rg_cd,reg_name,tr_cd,ter_name,"+grp+",sum(lysval) clysval,sum(tarval) ctarval,sum(salval) csalval  "+
			" from "+tblnm+" where mkt_year = ? and  mnth_code <= ?  and "+grp+" = ? "+
			" and depo_CODE =? "+
			" group by ar_cd,area_name,rg_cd,reg_name,tr_cd,ter_name,"+grp+") as b "+
...
////////////////// Month File Loop Starts to accumulate data/////////////////////////
		...
		{
			System.out.println("========Exception in SQLForm2DAO.getAllHQForm2 " + e); 
		}
		  finally 
		{
			try
			...
			{
				System.out.println("-------------Exception in SQLForm2DAO.Connection.close "+e);
			}
		}
